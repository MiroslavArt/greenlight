        ...
        $.ajax({
            ...
            success: function(data){
                ...
                if(data.error) {
                    ...
                    var lidescOne =  $("<div></div>").attr('class', 'doc_list_title').text('Срок предоставления')
                    var lispanOne = $("<span></span>").attr('class', 'name_format').text(curdate)
                    lidivOne.append(lidescOne)
                    lidivOne.append(lispanOne)
                    liblock.append(lidivOne)
                    var lidivTwo = $("<div></div>").attr('class', 'doc_format')
                    var lidescTwo = $("<div></div>").attr('class', 'doc_list_title').text('Автор')
                    var lispanTwo    = $("<span></span>").attr('class', 'name_format').text(data.success.uname)
                    lidivTwo.append(lidescTwo)
                    lidivTwo.append(lispanTwo)
                    liblock.append(lidivTwo)
                    var lidivThree = $("<div></div>").attr('class', 'doc_format')
                    var lidescThree = $("<div></div>").attr('class', 'doc_list_title').text('Описание')
                    var lihrefThree = $('<a>').attr('href', data.success.tmp_name).attr('download',data.success.name).text(newcommenttxt)
                    lidivThree.append(lidescThree)
                    lidivThree.append(lihrefThree)
